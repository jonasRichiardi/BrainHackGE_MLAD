...
One way to do this is to get a prediction for the last available observation of a subject in LB2, and forecast this same value for all 84 months.
...
df_all_results<-plyr::ldply(unique(dplyr::filter(df_tr, LB2==1)$RID), .fun=function(this_RID){
  ...
  df_res$CNRelativeProbability=df_impute[imp_rowidx_use,]$imp.CNRelativeProbability
  df_res$MCIRelativeProbability=df_impute[imp_rowidx_use,]$imp.MCIRelativeProbability
  df_res$ADRelativeProbability=df_impute[imp_rowidx_use,]$imp.ADRelativeProbability
  df_res$ADAS13=df_impute[imp_rowidx_use,]$imp.ADAS13
  ...
  df_res$Ventricles_ICV=df_impute[imp_rowidx_use,]$imp.Ventricles_ICV
...
name_submission<-'conditionalImputation.pre201004'
